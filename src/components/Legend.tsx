        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", marginBottom: "20px" }}>
            <div style={{ position: "relative", width: calculateCircleSize(100), height: calculateCircleSize(100), marginRight: "20px" }}>
                {sizes.map((item, index) => (
                    <div key={index} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: item.size, height: item.size, backgroundColor: "rgba(0, 0, 255, 0.3)", borderRadius: "50%" }}>
                        <span style={{ position: "absolute", bottom: "100%", left: "50%", transform: "translateX(-50%)", fontSize: "10px", whiteSpace: "nowrap" }}>{item.label}</span>
                    </div>
                ))}
                <span style={{ position: "absolute", bottom: "110%", left: "50%", transform: "translateX(-50%)", fontSize: "10px" }}>casualties</span>
            </div>
